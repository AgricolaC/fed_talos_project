...
def train_federated(config):
    ...
    if isinstance(model_cfg, str):
        if model_cfg not in MODEL_REGISTRY:
            raise KeyError(
                f"[train_federated] Unknown model string '{model_cfg}'. "
    ...
    if split == "iid":
        ...
        client_indices = noniid_partition(
            train_set, config["num_clients"], config["classes_per_client"]
        )
    else:
    ...
    # 4. build clients
    clients = [
        Client(Subset(train_set, idxs), config, cid=cid)
        for cid, idxs in client_indices.items()
    ]

    # sanity-check
    for cid, cl in enumerate(clients):
        assert isinstance(cl.dataset, torch.utils.data.Dataset)
